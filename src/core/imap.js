...
export default class Imap {
	...
	async getInboxFolder() {
		...
		for (const label of ['inbox', 'all mail']) {
			const inboxFolder = this.folders.find((item) => item.label.toLowerCase() === label);
			if (inboxFolder) {
	...
	async getPriboxFolder() {
		...
		for (const label of ['pribox']) {
			const priboxFolder = this.folders.find((item) => item.label.toLowerCase() === label);
			if (priboxFolder) {
	...
	async getSpamFolder() {
		...
		for (const label of ['spam', 'junk', 'bulk mail']) {
			const spamFolder = this.folders.find((item) => item.label.toLowerCase() === label);
			if (spamFolder) {
				return spamFolder;
			}
		}
