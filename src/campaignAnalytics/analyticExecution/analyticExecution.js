...
export class AnalyticExecution {
	...
	async init() {
		console.log(`campaign analytic Execution init for campaign ${this.campaign.name}`);
	...
	async startExecution() {
		console.log(`campaign analytic Execution startExecution for campaign ${this.campaign.name}`);
	...
	get imapConnection() {
		return new Promise(async (resolve, reject) => {
			try {
				const imap = await Imap.connect({ mailbox: this.sender });
	...
	async emailReplies(messageId) {
		...
		if (this.sender.provider === 'outlook') {
			...
			const response = await client
				...
				.get();
			if (response) {
				const paginator = new PageIterator(client, response, (message) => {
		...
		const inboxEmails = await this.imap.search([['HEADER', 'In-Reply-To', messageId]]);
